	...
	<div>
		<div class="home_box">
			...
			<div class="homeList_view">
				<div class="homeList_title">酒店信息展示</div>
				<div class="home_list_five">
					<div v-for="(item,index) in jiudianxinxiHomeList" :class="'home_list_five_item' + (Number(index) + 1)">
						<div class="img_box">
							<img class="home_img" v-if="isHttp(item.fangjiantupian)" :src="item.fangjiantupian.split(',')[0]" alt="">
							<img class="home_img" v-else :src="item.fangjiantupian?$config.url + item.fangjiantupian.split(',')[0]:''" alt="">
						</div>
				...
				<div class="homeList_more_view" @click="moreClick('jiudianxinxi')">
					<span class="homeList_more_text">全部</span>
				</div>
			</div>
			<!-- 旅游资讯 -->
...
<script setup>
	...
	//酒店信息首页展示
	const jiudianxinxiHomeList = ref([])
	const getjiudianxinxiHomeList = () => {
		context?.$http({
			url: 'jiudianxinxi/list',
			method: 'get',
			params: {
				page: 1,
				limit: 6
			...
			jiudianxinxiHomeList.value = res.data.data.list
		})
	}
	//旅游资讯弹窗
...
<style lang="scss">
	...
	.recomList_view {
		...
		.animation_box:hover {
			...
			transform: rotate(0deg) scale(1) skew(0deg, 0deg) translate3d(0px, -3px, 0px);
		...
		.recommend_more_view {
			...
			.recommend_more_text {
				padding: 0 16px 0 0;
				color: #ef4238;
				background: url(http://clfile.zggen.cn/20231214/87790c3e4a1d4685a636389bf810fe43.png) no-repeat right center;
				font-size: 16px;
			}
		}
	}
	...
	.newsList_view {
		...
		.news_more_view {
			...
			.news_more_text {
				padding: 0 16px 0 0;
				color: #ffb400;
				background: url(http://clfile.zggen.cn/20231214/f61ce3a2d75848c48d42317e08098c1d.png) no-repeat right center;
	...
	.homeList_view {
		...
		.homeList_more_view {
			...
			.homeList_more_text {
				padding: 0 16px 0 0;
				color: #2d98f3;
				background: url(http://clfile.zggen.cn/20231214/7e9d18d133854d009b2ed0fe4be90205.png) no-repeat right center;
				display: inline-block;
				font-size: 16px;
			}
		}
