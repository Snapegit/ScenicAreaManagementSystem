	...
	<div class="app-contain" :style='{"padding":"0 12%","margin":"10px auto 20px","alignItems":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">房间类型</div>
					<div  class="info_text" >{{detail.fangjianleixing}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">酒店类型</div>
					<div  class="info_text" >{{detail.jiudianleixing}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">房间地址</div>
					<div  class="info_text" >{{detail.fangjiandizhi}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">一晚价格</div>
					<div  class="info_text" >{{detail.yiwanjiage}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">预定天数</div>
					<div  class="info_text" >{{detail.yudingtianshu}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">总价</div>
					<div  class="info_text" >{{detail.zongjia}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">服务电话</div>
					<div  class="info_text" >{{detail.fuwudianhua}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">预定时间</div>
					<div  class="info_text" >{{detail.yudingshijian}}</div>
				</div>
				<div class="info_item">
				...
				<div class="btn_view">
					<el-button class="approval_btn" v-if="btnAuth('jiudianyuding','审核')" type="warning" @click="approvalClick()">审核</el-button>
					<el-button v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnFrontAuth('jiudianyuding','支付')" class="approval_btn" @click="payClick">支付</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('jiudianyuding','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('jiudianyuding','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
			...
			title.value = res.data.data.fangjianmingcheng
	...
	const pay = ref(null)
	const payClick = ()=>{
		if(detail.value.sfsh!='是'){
			context?.$toolUtil.message('请审核通过后再支付', 'error')
			return false
		}
