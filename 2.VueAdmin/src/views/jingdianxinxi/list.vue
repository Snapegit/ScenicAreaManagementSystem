	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('jingdianxinxi','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('jingdianxinxi','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('jingdianxinxi','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('jingdianxinxi','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column
					...
					label="门票链接">
					<template #default="scope">
						<span @click="linkOthers(scope.row.menpiaolianjie)">{{scope.row.menpiaolianjie}}</span>
					</template>
				</el-table-column>
				<el-table-column
					 :resizable='true' 
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('jingdianxinxi','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('jingdianxinxi','购买')" type="success" @click="menpiaogoumaiCrossAddOrUpdateHandler(scope.row,'cross','','','')">购买</el-button>
						<el-button v-if="btnAuth('jingdianxinxi','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussjingdianxinxi?refid=' + id)
	}
    ...
    const menpiaogoumaiCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			menpiaogoumaiFormModelRef.value.init(row.id,'cross','购买',row,'jingdianxinxi',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
