	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('jiudianxinxi','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('jiudianxinxi','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('jiudianxinxi','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('jiudianxinxi','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="房间图片" width="120" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('jiudianxinxi','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('jiudianxinxi','预定')" type="success" @click="jiudianyudingCrossAddOrUpdateHandler(scope.row,'cross','','','')">预定</el-button>
						<el-button v-if="btnAuth('jiudianxinxi','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<jiudianyudingFormModel ref="jiudianyudingFormModelRef" @formModelChange="formModelChange"></jiudianyudingFormModel>
	</div>
</template>
<script setup>
	...
	const commentClick=(id)=>{
		context?.$router.push('/discussjiudianxinxi?refid=' + id)
	}
	import jiudianyudingFormModel from '@/views/jiudianyuding/formModel'
	const jiudianyudingFormModelRef = ref(null)
    const jiudianyudingCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			jiudianyudingFormModelRef.value.init(row.id,'cross','预定',row,'jiudianxinxi',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
