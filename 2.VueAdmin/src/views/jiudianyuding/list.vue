...
<template>
	<div>
			...
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.fangjianleixing" placeholder="房间类型"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('jiudianyuding','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('jiudianyuding','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('jiudianyuding','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('jiudianyuding','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('jiudianyuding','酒店预定统计')">酒店预定统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column prop="ispay" label="支付状态" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						...
						<el-button v-if="scope.row.ispay!='已支付' && btnAuth('jiudianyuding','支付')" type="text" size="small" @click.stop="payClick(scope.row)">支付</el-button>
					</template>
				</el-table-column>
				...
				<el-table-column label="审核" v-if="btnAuth('jiudianyuding','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('jiudianyuding','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'酒店预定统计'" width="70%">
			<div  id="yudingtianshuEchart1" style="width:100%;height:600px;"></div>
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var yudingtianshuEchart1 = echarts.init(document.getElementById("yudingtianshuEchart1"),'macarons');
			context?.$http({
				url: `jiudianyuding/value/fangjianmingcheng/yudingtianshu`,
				method: 'get'
			}).then(res=>{
				...
				for(let i=0;i<obj.length;i++){
				    xAxis.push(obj[i].fangjianmingcheng);
				    yAxis.push(parseFloat((obj[i].total)));
				    pArray.push({
				        value: parseFloat((obj[i].total)),
				        name: obj[i].fangjianmingcheng
				    })
				}
				var option = {};
                option = {
				...
				// 使用刚指定的配置项和数据显示图表。
				yudingtianshuEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    yudingtianshuEchart1.resize();
				};
			})
		})
	}
	...
	const approvalClick = (row) => {
		let params = {
			...
			jiudianleixing: row.jiudianleixing,
			fangjiandizhi: row.fangjiandizhi,
			yiwanjiage: row.yiwanjiage,
			yudingtianshu: row.yudingtianshu,
			zongjia: row.zongjia,
			fuwudianhua: row.fuwudianhua,
