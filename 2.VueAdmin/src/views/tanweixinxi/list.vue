	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.tanweimingcheng" placeholder="摊位名称"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('tanweixinxi','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('tanweixinxi','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('tanweixinxi','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('tanweixinxi','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column
					 ...
					 align="left" 
					 header-align="left"
					 prop="tanweimingcheng"
					label="摊位名称">
					<template #default="scope">
						{{scope.row.tanweimingcheng}}
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('tanweixinxi','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('tanweixinxi','入住申请')" type="success" @click="ruzhushenqingCrossAddOrUpdateHandler(scope.row,'cross','','zhuangtai','已出租','已出租,未出租'.split(',')[0])">入住申请</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<ruzhushenqingFormModel ref="ruzhushenqingFormModelRef" @formModelChange="formModelChange"></ruzhushenqingFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.tanweimingcheng&&searchQuery.value.tanweimingcheng!=''){
			params['tanweimingcheng'] = '%' + searchQuery.value.tanweimingcheng + '%'
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	import ruzhushenqingFormModel from '@/views/ruzhushenqing/formModel'
	const ruzhushenqingFormModelRef = ref(null)
    const ruzhushenqingCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			ruzhushenqingFormModelRef.value.init(row.id,'cross','入住申请',row,'tanweixinxi',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
